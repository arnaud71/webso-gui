...
angular.module('websoApp')
  .controller('SolrCtrl', function ($scope,$resource,cfg) {

    $scope.solr = $resource(cfg.urlDB+'solr/collection1/:action',
      {action:'select', q:'', wt:'json' , 'json.wrf':"JSON_CALLBACK"},
      {get:{method:'JSONP'}});
...
angular.module('websoApp')
  .controller('SourceDataCtrl', function ($scope,$resource,cfg) {
    ...
    $scope.sourceData = $resource(cfg.urlServices+':action',
      {action:'get.pl', type_s:'source',user_s:'user_1',callback:"JSON_CALLBACK"},
      {get:{method:'JSONP'}});
